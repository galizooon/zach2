...
Texture::Texture(std::shared_ptr<SDL_Renderer> &renderer, const char *filename) :_renderer(renderer)
{
	_texture=std::shared_ptr<SDL_Texture>(IMG_LoadTexture(ren(), filename), SDL_DestroyTexture);

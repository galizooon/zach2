...
LayerTexture::LayerTexture(std::shared_ptr<SDL_Renderer> &renderer, const char *filename, double spd): Texture(renderer, filename), _speed(spd){
...
{
	_offset+=_speed;
	while(_offset>=width()) _offset-=width();
}
